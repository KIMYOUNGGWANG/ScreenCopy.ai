...
export function WizardForm({ onGenerate, isGenerating }: WizardFormProps) {
  ...
  const renderStep = () => {
    switch (step) {
      ...
      default: return <Step1 formData={formData} setFormData={setFormData} />
    }
  }
  ...
  return (
    <div className="space-y-4 py-4">
      ...
      <div className="space-y-2">
        ...
        <Input id="appName" value={formData.appName} onChange={(e) => setFormData(prev => ({ ...prev, appName: e.target.value }))} />
      </div>
      <div className="space-y-2">
        ...
        <Select value={formData.appCategory} onValueChange={(value) => setFormData(prev => ({ ...prev, appCategory: value }))}>
          <SelectTrigger id="appCategory"><SelectValue /></SelectTrigger>
          <SelectContent>
...
function Step3({ formData, setFormData }: StepProps) {
  const setAudience = (field: keyof GenerateRequest['targetAudience'], value: string) => {
    ...
    <div className="space-y-4 py-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={formData.targetAudience.occupation} onValueChange={(v) => setAudience("occupation", v)}><SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
      ...
      <div className="space-y-2">
        ...
        <Textarea value={formData.targetAudience.painPoint} onChange={(e) => setAudience("painPoint", e.target.value)} placeholder="e.g., I keep forgetting my tasks and missing deadlines." />
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-4 py-4">
      ...
      <div className="space-y-2">
        ...
        <Input value={formData.keyBenefit} onChange={(e) => setFormData(prev => ({ ...prev, keyBenefit: e.target.value }))} placeholder="e.g., See all your tasks at a glance" />
      </div>
      <div className="space-y-2">
        <Label>Competitors (optional, comma-separated)</Label>
        <Input value={formData.competitors} onChange={(e) => setFormData(prev => ({ ...prev, competitors: e.target.value }))} placeholder="e.g., Todoist, Any.do" />
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-4 py-4">
      ...
      <div className="space-y-2">
        ...
        <Select value={formData.tonePreference} onValueChange={(value) => setFormData(prev => ({ ...prev, tonePreference: value }))}>
          <SelectTrigger><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="professional">Professional</SelectItem>
